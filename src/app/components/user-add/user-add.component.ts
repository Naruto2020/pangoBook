...
export class UserAddComponent implements OnInit {
  ...
  addusr = new FormGroup({
    idToFollow : new FormControl('')
       
  });

  rmvusr = new FormGroup({
  ...
  // follow
  friend(){
    console.log("idTofollow", this.addusr.value);
    console.log("req.paramsId", this.myId);

    this.crud.followUsr(this.myId, this.addusr.value).subscribe(res =>{
      console.log(res);
      return res;
    });
  }
  // unfollow
  nofriend(){
    this.crud.unfollowUsr(this.myId, this.rmvusr.value).subscribe(res =>{
      console.log(res)
      return res;
    });
  }
